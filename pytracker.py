...
Example
	...
	%s '/home/' '.php' -ignore '/home/path_to_file'   - to ignore file
	%s '/home/' '.php' -unignore '/home/path_to_file' - to remote file from ignore list
	%s '/home/' '.php' -ignore-list                   - to list all ignored files
...
if otherArguments == 'history':
        ...
        if len(sys.argv) == 4:
                ...
                for i in curr.execute(" SELECT COUNT(*), h.runid, h.created  FROM history h INNER JOIN files f ON f.id = h.file_id GROUP BY h.runid ORDER BY h.created ASC"):
...
if otherArguments == 'ignore-list':
	print "Files ignored:"
	for i in curr.execute(" SELECT path FROM files WHERE ignore = 1"):
		print "%s" % i[0]
	quit()

if otherArguments == 'ignore':
	if (len(sys.argv) != 5):
		print "Need file path to ignore"
	else:
		print "Ignoring file %s " % sys.argv[4]
		curr.execute("UPDATE files SET ignore = 1 WHERE path = ? ", (sys.argv[4], ))

	conn.commit();
	conn.close();
	quit();

if otherArguments == 'unignore':
        if (len(sys.argv) != 5):
                print "Need file path to unignore"
        else:
		print "Unignoring file %s " % sys.argv[4]
                curr.execute("UPDATE files SET ignore = 0 WHERE path = ? ", (sys.argv[4], ))
        conn.commit();
        conn.close();
        quit();
...
for root, subdirs, files in os.walk(targetDir):
        for f in files:
				...
				f.ignore
                        ...
                        elif (result[2] != hashed) and (result[4] == 0):
